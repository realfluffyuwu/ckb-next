...
#define IS_SCROLLWHEEL_V(scan)                                                 \
  ((scan) == BTN_WHEELUP || (scan) == BTN_WHEELDOWN)
#define IS_SCROLLWHEEL_H(scan)                                                 \
  ((scan) == BTN_WHEELLEFT || (scan) == BTN_WHEELRIGHT)
#define IS_VOLWHEEL(scan)                                                      \
  (((scan) == KEY_VOLUMEUP || (scan) == KEY_VOLUMEDOWN) && DEV_HAS_VOLWHEEL(kb))
...
///
/// \brief pt_head is the head pointer for the single linked thread list managed
/// by macro_pt_en/dequeue().
static ptlist_t *pt_head = 0;
/// \brief pt_tail is the tail pointer for the single linked thread list managed
/// by macro_pt_en/dequeue().
static ptlist_t *pt_tail = 0;

// FIXME: Most of the following macro queueing code is wrong.
// We can't assume pthread_t is an arithmetic type, so assigning it to 0 is
// incorrect.

///
/// \brief macro_pt_enqueue Save the new thread in the single linked list
/// (FIFO).
...
static void macro_pt_enqueue() {
  ...
  if (!new_elem) {
    ...
    exit(-1); ///< exit on critical situation; \todo find a better exit strategy
              ///< if no more mem available.
  }
  new_elem->next = 0;
...
///
/// \brief macro_pt_dequeue gets the first thread id of the list and returns the
/// thread_id stored in it.
...
static pthread_t macro_pt_dequeue() {
  ...
  if (pt_head == 0)
    pt_tail = 0;            ///< Was last element in the list, so clear tail.
...
///
/// \brief macro_pt_first returns the first pthread_id but does not remove the
/// first entry.
/// \return the pthread_id of the first element in the list or 0 if list is
/// empty.
/// \attention Becuase multiple threads may use this function in parallel,
/// save the critical section with a mutex (avoid NIL-ptr)
///
static pthread_t macro_pt_first() { return pt_head ? pt_head->thread_id : 0; }

// Default macro keystroke delay
const struct timespec macrodelay = {.tv_nsec = 1000000};
// Initial repeat delay
#define DELAY_REPEAT_INITIAL 80000000
// Delay for every subsequent repeat
#define DELAY_REPEAT_CATCHUP 80000000
...
/// \param param \a macro_param to store Kb-ptr and macro-ptr (thread may get
/// only one user-parameter)
/// \return 0 on success, -1 else (no one is interested in it except the
/// kernel...)
///
static void *play_macro(void *param) {
  ...
  /// First have a look if we are the first and only macro-thread to run. If
  /// not, wait. So enqueue our thread first, so it is remembered for us and can
  /// be seen by all others.
  ...
  while (macro_pt_first() !=
         pthread_self()) { ///< If the first thread in the list is not our,
                           ///< another one is running
    // ckb_info("Now waiting with 0x%lx because of 0x%lx", (unsigned long
    // int)pthread_self(), (unsigned long int)macro_pt_first());
    pthread_cond_wait(mvar(kb), mmutex2(kb));
    // ckb_info("Waking up with 0x%lx", (unsigned long int)pthread_self());
  }
  pthread_mutex_unlock(
      mmutex2(kb)); ///< Give all new threads the chance to enter the block.
  ...
  while (1) {
    ...
    queued_mutex_lock(mmutex(
        kb)); ///< Synchonization between macro output and color information
    for (int a = 0; a < ptr->actioncount; a++) {
      ...
      if (action->rel_x != 0 || action->rel_y != 0) {
        ...
        queued_mutex_unlock(
            mmutex(kb)); ///< use this unlock / relock for enablling the
                         ///< parallel running colorization
        if (action->delay != UINT32_MAX && action->delay) { ///< local delay set
          if (action->delay_max) {
            ...
            final |= (second << 1);
            // Now that we have a sufficently large random number, range it
            // appropriately
            final = ((uint64_t)action->delay + (uint64_t) final) %
                    (action->delay_max - action->delay);
    ...
    int delay_ns =
        first_keydownloop ? DELAY_REPEAT_INITIAL : DELAY_REPEAT_CATCHUP;
    ...
    // if the key released and pressed during the sleep, reset to use
    // DELAY_REPEAT_INITIAL
    if (macro->triggered == 1)
      first_keydownloop = 0;
  ...
  // ckb_info("Now leaving 0x%lx and waking up all others", (unsigned long
  // int)pthread_self());
  macro_pt_dequeue();
  ...
  queued_mutex_unlock(
      mmutex(kb)); ///< Sync keyboard input/output and colorization
  free(param);
  return 0;
}

// Checks if the macro mask contains any wheels to prevent it from looping
// endlessly
static inline int is_wheel_keybit(const usbdevice *kb, const uchar *macro) {
  for (int i = 0; i < N_KEYBYTES_INPUT; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      if (IS_VOLWHEEL(ckey->scan) || IS_SCROLLWHEEL_V(ckey->scan) ||
          IS_SCROLLWHEEL_H(ckey->scan))
        return 1;
    }
  }
  return 0;
...
///
/// \brief inputupdate_keys Handle input from Keyboard or mouse; start Macro if
/// detected.
/// \param kb
///
static inline void inputupdate_keys(usbdevice *kb, int *sync_kb,
                                    int *sync_mouse) {
  ...
  if (kb->active) {
    for (int i = 0; i < bind->macrocount; i++) {
      ...
      if (macromask(input->keys, macro->combo)) {
        if (!(macro->triggered & 1)) {
          macro->triggered++;
          // Because there are no keyup events for wheels, pretend we received
          // them to prevent endless looping
      ...
      if (macro->triggered <= 3) {
        ...
        if (!macro->param) {
          ...
          // if there is already a thread running, it may be waiting for
          // DELAY_REPEAT_* it does not need to wait anymore
          pthread_cond_broadcast(mintvar(kb));
        }
      }
    }
  }
  // Make a list of keycodes to send. Rearrange them so that modifier keydowns
  // always come first and modifier keyups always come last. This ensures that
  // shortcut keys will register properly even if both keydown events happen at
  // once.
  // + 2 is used because the volume wheel generates keydowns and keyups at the
  // same time
  // + (-SCHAR_MIN) * 2 * 2 is used because a mouse wheel can generate up to
  // 128*2 events at once, one for x, and one for y to be safe
  static_assert(sizeof(input->whl_rel_x) == sizeof(char) &&
                    sizeof(input->whl_rel_y) == sizeof(char),
                "Please change SCHAR_MIN below to the one for the appropriate "
                "type used by the scroll wheel.");
    ...
    for (int bit = 0; bit < 8; bit++) {
      ...
      if (old != new) {
        ...
        if (!(scancode & SCAN_SILENT)) {
          if (IS_MOD(scancode)) {
            // Only keyboards have modifiers, so ask explicitly for a sync if we
            // only have a modifier in the queue
            *sync_kb = 1;
            if (new) {
            ...
            // Simulate scrolling if this came from a macro or a device without
            // an axis (keyboard)
            if (!input->whl_rel_y)
            ...
            *sync_mouse = 1;
            ...
            // The only exception is binding the vertical scroll wheel to the
            // horizontal one
            ...
            *sync_mouse = 1;
            ...
            if (scancode & SCAN_MOUSE)
              ...
              *sync_kb = 1;
            memmove(events + modcount + keycount + 1,
                    events + modcount + keycount, sizeof(int) * rmodcount);
            events[modcount + keycount++] =
                new ? (scancode + 1) : -(scancode + 1);
          }
        }

        // The volume/scroll wheels don't generate keyups, so create them
        // automatically when necessary
        if (new) {
          if (scancode != KEY_UNBOUND) {
            // If it's a volume wheel, just add a single keyup event to the FIFO
            // Else if the event originated from a scroll wheel, and is bound to
            // something other than a scroll wheel, multiply the last few events
            // by the amount of events received from the hid report
            if (IS_VOLWHEEL(map->scan)) {
              ...
              if (!IS_SCROLLWHEEL_V(scancode)) {
                ...
                // Make room for the extra events. We already have one, thus
                // - 1.
                memmove(events + modcount + keycount + rel_y * 2 - 1,
                        events + modcount + keycount, sizeof(int) * rmodcount);
                // Duplicate the last event, but with keyup
                events[modcount + keycount++] = -(scancode + 1);
                ...
                for (int i = 0; i < rel_y - 1; i++) {
                  memcpy(events + modcount + keycount,
                         events + modcount + keycount - 2, sizeof(int) * 2);
                  keycount += 2;
                }
              }
            }
          }
          // Clear the key bit so that we can receive events of this type in the
          // next run
        ...
        if (kb->active) {
          for (int notify = 0; notify < OUTFIFO_MAX; notify++) {
            if (mode->notify[notify][byte] & mask) {
              // Wheels don't generate keyups and additionally can produce
              // multiple events with a single report. No device so far supports
              // scrolling horizontally, so only vertical notifications are
              // implemented.
              if (IS_SCROLLWHEEL_V(map->scan)) {
  ...
  for (int i = 0; i < totalkeys; i++) {
    ...
    // Force a keyboard sync if the event queue contains two of the same
    // scancode before the second keypress is sent This can't happen in mice
    // because the wheel is an axis
...
void updateindicators_kb(usbdevice *kb, int force) {
  ...
  if (old != new || force) {
    ...
    int len = 1;
    if ((kb->protocol == PROTO_NXP &&
         (kb->fwversion >= 0x300 || IS_V3_OVERRIDE(kb))) ||
        kb->protocol == PROTO_BRAGI) {
      leds = (kb->ileds << 8) | 0x0001;
      len = 2;
    }
    queued_mutex_unlock(dmutex(kb));
    ctrltransfer transfer = {.bRequestType = 0x21,
                             .bRequest = 0x09,
                             .wValue = 0x0200,
                             .wIndex = 0,
                             .wLength = len,
...
void cmd_bind(usbdevice *kb, usbmode *mode, int dummy, int keyindex,
              const char *to) {
  (void)dummy;
...
void cmd_unbind(usbdevice *kb, usbmode *mode, int dummy, int keyindex,
                const char *to) {
  (void)dummy;
  (void)to;
...
void cmd_rebind(usbdevice *kb, usbmode *mode, int dummy, int keyindex,
                const char *to) {
  (void)dummy;
  (void)to;
...
static void _cmd_macro(usbmode *mode, const char *keys, const char *assignment,
  ...
  char keyname[40];
  while (position < left &&
         sscanf(keys + position, "%10[^+]%n", keyname, &field) == 1) {
  ...
  position = 0;
  field = 0;
  // max action = old 11 chars plus 12 chars which is the max 32-bit unsigned
  // int 4294967295 size * 2 + 1 for range underscore
  while (position < right &&
         sscanf(assignment + position, "%36[^,]%n", keyname, &field) == 1) {
    ...
    uint32_t delay = UINT32_MAX; // computed delay value. UINT32_MAX means use
                                 // the default value.
    uint32_t delay_range = 0;
    char real_keyname[12]; // temp to hold the left side (key) of the
                           // <key>=<delay>
    int scan_matches = sscanf(keyname, "%11[^=]=%" SCNd64 "_%" SCNd64,
                              real_keyname, &long_delay, &long_delay_range);
  ...
  if (bind->macrocount >= bind->macrocap)
    bind->macros =
        realloc(bind->macros, (bind->macrocap += 16) * sizeof(keymacro));
}

void cmd_macro(usbdevice *kb, usbmode *mode, const int notifynumber,
               const char *keys, const char *assignment) {
  (void)notifynumber;
